...
const toXMLTVJSON = xml => xml2json.toJson(xml, {
    sanitize: false,
    object: true
});


// read cache for fast start
const CHANNELS_CACHE = "./cache/channels.json";
const PROGRAMS_CACHE = "./cache/programs.json";
let cachedChannels, chachedPrograms;
try {
    cachedChannels = jsonfile.readFileSync(CHANNELS_CACHE);
    chachedPrograms =jsonfile.readFileSync(PROGRAMS_CACHE);
} catch (e) {}
...
let tvGuide = {
    ...
    _updateFromXMTLTVJSON(json) {
        ...
        jsonfile.writeFile(CHANNELS_CACHE, channels);
        jsonfile.writeFile(PROGRAMS_CACHE, programs);
    }
};
